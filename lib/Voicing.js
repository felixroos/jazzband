...
var Voicing = /** @class */ (function () {
    ...
    Voicing.getNextVoicing = function (chord, lastVoicing, range, maxVoices) {
        if (range === void 0) { range = ['C3', 'C5']; }
        if (maxVoices === void 0) { maxVoices = 4; }
        // make sure tonal can read the chord
        chord = util_1.getTonalChord(chord);
        if (chord === 'r') {
            return null;
        }
        // get chord notes
        // const notes = getVoices(chord, 5, false, 0);
        var notes = Voicing.getVoices(chord, maxVoices, false, 0);
        ...
        if (!lastVoicing || !lastVoicing.length) { // no previous chord
            ...
            var firstPick = util_1.randomElement(combinations);
            var firstNoteInRange = util_1.getNearestNote(range[0], firstPick[0], 'up');
    ...
    Voicing.getAllChoices = function (combinations, lastVoicing) {
        ...
        return combinations.map(function (combination) { return ({
            ...
            return {
                combination: combination,
                bottomNotes: [
                    tonal_3.Distance.transpose(lastVoicing[0], util_1.minInterval(bottomInterval, 'down')),
                    tonal_3.Distance.transpose(lastVoicing[0], util_1.minInterval(bottomInterval, 'up')),
                ]
            };
    ...
    Voicing.voiceLeading = function (chordA, chordB) {
        ...
        return Permutation_1.Permutation.binomial(chordA, chordB.length)
            ...
            var intervals = Voicing.voicingIntervals(from, to, false)
                .map(function (interval) { return util_1.fixInterval(interval, false); });
            var degrees = intervals.map(function (i) { return util_1.getDegreeFromInterval(i); });
            var oblique = origin.filter(function (n, i) { return targets.find(function (note) { return util_1.isSameNote(n, note); }); });
            var dropped = [], added = [];
    ...
    Voicing.voicingIntervals = function (chordA, chordB, min, direction) {
        ...
        var intervals = chordA.map(function (n, i) {
            ...
            if (util_1.isPitchClass(n) && util_1.isPitchClass(chordB[i])) {
                return util_1.minInterval(interval, direction);
            }
            return interval;
        });
        return intervals;
