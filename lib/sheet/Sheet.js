...
var Sheet = /** @class */ (function () {
    ...
    Sheet.from = function (sheet) {
        ...
        return __assign({}, sheet, { options: __assign({ forms: 1, arpeggio: false, bell: false, pedal: false, real: true, tightMelody: true, bpm: 120, swing: 0, fermataLength: 4, duckMeasures: 1 }, sheet.options, { humanize: __assign({ velocity: 0.1, time: 0.002, duration: 0.002 }, (sheet.options.humanize || {})), voicings: __assign({ minBottomDistance: 3, minTopDistance: 2, logging: false, maxVoices: 4, range: ['C3', 'C6'], rangeBorder: [0, 0], maxDistance: 7 }, (sheet.options.voicings || {})) }) });
    };
    Sheet.render = function (sheet, options) {
        if (options === void 0) { options = {}; }
        var state = __assign({ sheet: sheet, measures: [], forms: 1, property: 'chords', nested: true, fallbackToZero: true, firstTime: true }, options);
        state = __assign({}, state, { totalForms: state.forms }, Sheet.newForm(state));
        var runs = 0;
    ...
    Sheet.nextMeasure = function (state) {
        ...
        var sheet = state.sheet, index = state.index, measures = state.measures, property = state.property;
        state = __assign({}, state, { measures: measures.concat([Measure_1.Measure.render(state)]) }, Sheet.nextIndex(state));
