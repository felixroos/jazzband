...
test('rule 1: repeat end without start', function () {
    expect(Snippet_1.testFormat(Sheet_1.Sheet.render([
        { chords: ['A'] },
        { chords: ['B'], signs: ['}'] },
    ]))).toEqual('A B A B');
});
...
test('repeat one bar in the middle', function () {
    expect(Snippet_1.testFormat(Sheet_1.Sheet.render(['A', { chords: ['B'], signs: ['{', '}'] }, 'C']))).toEqual('A B B C');
});
...
test('Sheet.render: multi houses', function () {
    expect(Snippet_1.testFormat(Sheet_1.Sheet.render(withMultiHouse))).toEqual('A B A B A C');
});
