...
function getJumpSign(measure) {
    var signs = (Bar.from(measure).signs || [])
...
function getAllowedJumps(_a) {
    ...
    var measure = Bar.from(sheet[index]);
...
function renderSheet(sheet, options) {
    ...
    return state.measures /* .map(m => Bar.from(m)) */;
}
