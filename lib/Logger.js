...
var Logger = /** @class */ (function () {
    ...
    Logger.logNotes = function (activeNotes, idleNotes, addedNotes, range) {
        ...
        var keyboard = allNotes.map(function (note, index) {
            var active = !!activeNotes.find(function (n) { return util_1.isSameNote(note, n); });
            var idle = !!idleNotes.find(function (n) { return util_1.isSameNote(note, n); });
            var added = !!addedNotes.find(function (n) { return util_1.isSameNote(note, n); });
            var black = util_1.isBlack(note);
            var css = '', sign = '_';
            if (added && !black) {
                css = 'color:green;';
                sign = '█';
    ...
    Logger.logVoicing = function (_a) {
        ...
        var idle = lastVoicing.filter(function (n) { return pick.find(function (p) { return util_1.isSameNote(n, p); }); });
        var active = pick.filter(function (n) { return !lastVoicing.find(function (p) { return util_1.isSameNote(n, p); }); });
