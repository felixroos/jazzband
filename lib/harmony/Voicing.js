    ...
    */
    ...
    Voicing.voicingValidator = function (options) {
        ...
        return function (path, next, array) {
            ...
            return Permutation_1.Permutation.combineValidators(Voicing.notesAtPositionValidator(options.topNotes, lastPosition), Voicing.notesAtPositionValidator(options.bottomNotes, 0), Voicing.degreesAtPositionValidator(options.topDegrees, lastPosition, options.root), Voicing.degreesAtPositionValidator(options.bottomDegrees, 0, options.root), Voicing.validateInterval(function (interval) { return tonal_4.Interval.semitones(interval) <= options.maxDistance; }), Voicing.validateInterval(function (interval, _a) {
    ...
    Voicing.getAllNoteSelections = function (chord, options) {
        ...
        var extraNotes = topNotes.concat(bottomNotes).map(function (n) { return tonal_2.Note.pc(n); });
    ...
    Voicing.getAllChoices = function (combinations, previousVoicing, options) {
        ...
        if (topNotes && topNotes.length) {
            var absoluteTopNotes = (topNotes || []).filter(function (n) { return !!tonal_2.Note.oct(n); });
