...
var Voicing = /** @class */ (function () {
    ...
    Voicing.getNextVoicing = function (chord, lastVoicing, options) {
        if (options === void 0) { options = {}; }
        var _a = __assign({ range: ['C3', 'C5'], rangeBorders: [3, 3], maxVoices: 4, forceDirection: null, maxDistance: 7, minBottomDistance: 3, minTopDistance: 2 }, options), maxVoices = _a.maxVoices, range = _a.range, forceDirection = _a.forceDirection, rangeBorders = _a.rangeBorders;
        // make sure tonal can read the chord
        chord = Harmony_1.Harmony.getTonalChord(chord);
        if (chord === 'r') {
            return null;
        }
        var combinations = Voicing.getVoicePermutations(chord, maxVoices, options);
        ...
        var exit = function () {
            var pick = [];
            Logger_1.Logger.logVoicing({ chord: chord, lastVoicing: lastVoicing, range: range, combinations: combinations, pick: pick });
            return pick;
        };
        ...
        if (!lastVoicing || !lastVoicing.length) { // no previous chord
            ...
            Logger_1.Logger.logVoicing({ chord: chord, lastVoicing: lastVoicing, range: range, combinations: combinations, pick: pick_1 });
            return pick_1;
        }
        var choices = Voicing.getAllChoices(combinations, lastVoicing, range);
        if (!choices.length) {
            return exit();
        }
        var bestPick = choices[0].targets, choice;
        var direction = Voicing.getDesiredDirection(lastVoicing, range, rangeBorders) || forceDirection;
        if (!direction) {
            var pick_2 = bestPick;
            choice = choices[0];
            Logger_1.Logger.logVoicing({ chord: chord, lastVoicing: lastVoicing, range: range, combinations: combinations, pick: pick_2, direction: direction, bestPick: bestPick, choice: choice, choices: choices });
            return pick_2;
        }
        ...
        Logger_1.Logger.logVoicing({ chord: chord, lastVoicing: lastVoicing, range: range, combinations: combinations, pick: pick, direction: direction, bestPick: bestPick, choice: choice, choices: choices });
        return pick;
    };
    ...
    Voicing.getNoteCombinations = function (chord, length) {
        if (length === void 0) { length = 2; }
        var required = Voicing.getRequiredNotes(chord);
        var fill = length - required.length;
        if (length === 1) {
        ...
        if (fill < 0) { // required notes are enough
            return Permutation_1.Permutation.binomial(required, length);
        }
    ...
    Voicing.getVoicePermutations = function (chord, length, voicingOptions) {
        return Voicing.getNoteCombinations(chord, length)
            .reduce(function (combinations, combination) {
            return combinations.concat(Voicing.getVoicingCombinations(combination, voicingOptions));
        }, []);
    };
    ...
    Voicing.getAllChoices = function (combinations, lastVoicing, range) {
        ...
        return combinations
            ...
            .reduce(function (all, notes) {
            var leads = Voicing.voiceLeading(lastVoicing, notes);
