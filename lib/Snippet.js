...
function renderChordSnippet(snippet) {
    ...
    return Sheet_1.renderSheet(parsed);
}
...
function testFormat(sheet) {
    return sheet.map(function (m) { return Sheet_1.getMeasure(m); }).map(function (m) { return m.chords; }).join(' ');
}
exports.testFormat = testFormat;
function getChordSnippet(sheet, format) {
    if (format === void 0) { format = true; }
    var snippet = sheet
        .map(function (m) { return Sheet_1.getMeasure(m); })
...
function expandSnippet(snippet) {
    ...
    var sheet = Sheet_1.renderSheet(rendered);
