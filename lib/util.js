...
function analyzeVoiceLeading(voicings, min) {
    ...
    return __assign({}, data, { averageMovement: data.movement / voicings.length, averageDifference: data.difference / voicings.length, averageVoiceDifference: data.voiceDifference / voicings.length });
}
...
function bestCombination(notes, combinations, direction) {
    if (direction) {
        var total = combinations.length;
        combinations = combinations.filter(function (current) {
            if (direction === 'up') {
...
// NEW
function generateVoicing(chord, lastVoicing, range) {
    if (range === void 0) { range = ['F3', 'F4']; }
    chord = getTonalChord(chord);
    var octave = 3;
    var pick = getChordNotes(chord /* , validateWithoutRoot */); // TODO: fix validateWithoutRoot
    var combinations, nextPitches;
    ...
    var nearest = getNearestNote(lastPitches[0], nextPitches[0]);
    var bottomOctave = tonal_1.Note.props(nearest).oct;
    return renderAbsoluteNotes(nextPitches, bottomOctave);
}
exports.generateVoicing = generateVoicing;
function getNextVoicing(chord, lastVoicing, range) {
    ...
    var nextPitches = bestCombination(lastPitches, combinations, force ? direction : null);
