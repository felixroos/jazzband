...
function getOptimalMovement(chordA, chordB) {
}
exports.getOptimalMovement = getOptimalMovement;
function voiceDifference(chordA, chordB) {
    return chordA.reduce(function (diff, note, index) {
        var notes = [note, chordB[index]];
        var min = minInterval(tonal_1.Distance.interval(notes[0], notes[1]));
        return diff + Math.abs(tonal_1.Interval.semitones(min));
    }, 0);
}
exports.voiceDifference = voiceDifference;
function voiceMovement(chordA, chordB) {
    return chordA.reduce(function (diff, note, index) {
        var min = minInterval(tonal_1.Distance.interval(note, chordB[index]));
        return diff + tonal_1.Interval.semitones(min);
    }, 0);
}
exports.voiceMovement = voiceMovement;
