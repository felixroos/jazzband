...
function parseChords(chords) {
    return chords
        ...
        .map(function (measure) { return measure.split(/\s/); }) // split measures by spaces
        .map(function (measure) { return measure.filter(function (chord) { return !!chord; }); }) // kill empty chords
        .map(function (measure) { return measure.length === 1 ? measure[0] : measure; }) // simplify one chord measures
        .filter(function (measure) { return !measure || measure.length; }); // kill empty measures
}
exports.parseChords = parseChords;
