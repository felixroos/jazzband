...
var Pattern = /** @class */ (function () {
    ...
    Pattern.nestIndices = function () {
        ...
        parent = this.flat(parent).map(function (i) { return children[0].map(function (p) {
            if (isNaN(p) || isNaN(i)) {
                return p + ' ' + i;
            }
            return p + i;
    ...
    Pattern.renderEvents = function (lines, options) {
        var flat = Sheet_1.Sheet.flatten(lines, true);
        var events = Sequence_1.Sequence.renderEvents(flat, options)
            .map(function (e) { return (__assign({}, e, { note: tonal_2.Note.simplify(e.value.note) })); });
        return events;
    };
    return Pattern;
}());
